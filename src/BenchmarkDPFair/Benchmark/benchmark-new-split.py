...
def _load_data(data_conf: BenchmarkDatasetConfig, filename: str, seed: int, epsilon: Optional[float] = None, 
    ...
    if (os.path.dirname(filename)):
        ...
        val_path  = os.path.dirname(os.path.dirname(filename)) # new 2026
    else:
        ...
        val_path = f"{data_conf.dir}/{data_conf.name}/{DP_ALGORITHM}/DP-dataset-{f'epsilon-{epsilon}-cal' if epsilon is not None else 'cal'}" # new 2026

    test_filename = f"{base_pattern[0]}_test{ext}"
    val_filename  = f"{val_path}/{base}{ext}" if epsilon is not None else f"{val_path}/{base.replace('_train', '_cal')}{ext}"  # new 2026
    ...
    if not os.path.exists(test_path) or not os.path.exists(test_path + "/" + test_filename):
        ...
        val_ds = pd.read_csv(val_filename, usecols=lambda col: col in cols) # new 2026

        if data_conf.index_col:
            test_ds.set_index(data_conf.index_col, inplace=True)
            val_ds.set_index(data_conf.index_col, inplace=True) # new 2026
    
        # Verify if data was read successfully
        read_verification(test_ds, data_conf.usecols)
        read_verification(val_ds, data_conf.usecols) # new 2026

        # Apply extra processing to dataset if the user wants it
        if extra_processing is not None:
            extra_processing(test_ds, **kwargs)
            extra_processing(val_ds, **kwargs) # new 2026
            
        X_test = test_ds.drop(columns=[data_conf.target])
        y_test = test_ds[data_conf.target]

        X_val = val_ds.drop(columns=[data_conf.target])
        y_val = val_ds[data_conf.target]

        # commented in 2026 April
    ...
    print(f"x_val and y_val has been taken from {val_filename}")
