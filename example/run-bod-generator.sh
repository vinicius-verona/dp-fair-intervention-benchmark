#!/bin/bash
# run-bod-generator.sh
# Usage: ./run-bod-generator.sh [seed1 seed2 ... seedX]
# RAM limit can be overridden via RAM_LIMIT_GB env var:
#   RAM_LIMIT_GB=4 ./run-bod-generator.sh 1 2 3
# COMBO can be overridden via COMBO env var:
#   COMBO=3 ./run-bod-generator.sh 1 2 3
...
echo "Running $JOBS jobs in parallel with a RAM limit of ${RAM_LIMIT_GB}GB each and combo $COMBO..." >> "$LOG_FILE"
