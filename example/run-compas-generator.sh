#!/bin/bash
# run-compas-generator.sh
# Usage: ./run-compas-generator.sh [seed1 seed2 ... seedX]
# RAM limit can be overridden via RAM_LIMIT_GB env var:
#   RAM_LIMIT_GB=4 ./run-compas-generator.sh 1 2 3
...
PYTHON_SCRIPT="./DPF_DataGeneration-Compas.py"
LOG_FILE="log/Parallel/killed_scripts-compas-gen.log"
...
run_with_ram_limit() {
    local seed="$1"
    local unit_name="Compas-DataGeneration-seed-${seed}-$$"
    local PYTHON="$VIRTUAL_ENV/bin/python3"

    systemd-run --user --scope --unit="$unit_name" -p MemoryMax="${RAM_LIMIT_GB}G" \
        "$PYTHON" "$PYTHON_SCRIPT" "-s $seed" > "log/DataGeneration/Compas-seed-${seed}.out" 2> "log/DataGeneration/Compas-seed-${seed}.err"
    local exit_code=$?
