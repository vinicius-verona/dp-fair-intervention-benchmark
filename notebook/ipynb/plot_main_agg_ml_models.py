...
#
# If automatic root detection fails, define manually:
#
...
SHOW_CI = True
CI_METHODS = "all"   # or a list such as ["DP-ONLY-XGB", "roc-XGB", "eqodds-XGB"]

# CIs larger than these limits are omitted from the figure to avoid
# visually dominating the Pareto plots. Their details are saved to CSV.
MAX_X_CI = 0.20      # maximum half-width on the fairness axis
MAX_Y_CI = 0.05      # maximum half-width on the utility axis
...
def _detect_seed_column(df):
    """
    Detect the column identifying independent random seeds.
    """
    ...
    raise ValueError(
        "No seed column found. Computing a 95% confidence interval "
        "requires per-seed results."
    )


def _mean_ci95(series):
    """
    Return the sample mean and the half-width of the two-sided
    ...
    ci95 = float(student_t.ppf(0.975, df=n - 1) * sem)
...
def _should_show_ci(method, xerr, yerr):
    """
    Decide whether the CI for a point should be drawn.
    """
    if not SHOW_CI:
        return False
...
if __name__ == "__main__":
        ...
        for COMBO in [1, 2, 3, 4, 5, 6]:
            for dataset in DATASETS:
                ...
                for ML_MODEL in ML_MODELS:
                    ...
                    # Keep only the columns needed to compute the means and CIs.
                    required_metric_columns = list(
                        dict.fromkeys(FAIRNESS_METRICS + [UTILITY_METRIC])
                    )
                    ...
                    missing_columns = [
                        ...
                        if col not in combined_raw_df.columns
                    ]

                    if missing_columns:
                        raise ValueError(
                            f"Missing required columns for CI computation: "
                            f"{missing_columns}"
                        )

                    combined_raw_df = combined_raw_df[required_columns].copy()

                    # Average duplicate rows belonging to the same independent seed,
                    # method, and privacy budget before computing the CI across seeds.
                    per_seed_df = (
                        combined_raw_df
                        .groupby(
                            ["Fair-Method", "Epsilon", seed_col],
                            dropna=False,
                        )[required_metric_columns]
                        .mean()
                        .reset_index()
                    ...
                    # ---- Compute mean and Student-t CI95 per method and epsilon ----
                    grouped_rows = []

                    for (method, eps), g in per_seed_df.groupby(
                        ["Fair-Method", "Epsilon"],
                        dropna=False,
                    ):
                        row = {
                ...
                for c, FAIRNESS_METRIC in enumerate(FAIRNESS_METRICS):
                    ...
                    for method in all_methods:
                        ...
                        for _, row in data.iterrows():
                            ...
                            if _should_show_ci(fair_method_ml, xerr, yerr):
                                ax.errorbar(
                                    x,
                                    y,
                                    xerr=xerr,
                                    yerr=yerr,
                                    fmt="none",
                                    ecolor=colors_map[fair_method_ml],
                                    elinewidth=1.0,
                                    capsize=2,
                                    alpha=0.15,
                                    zorder=4,
                                ...
                                and (
                                    CI_METHODS == "all"
                                    or fair_method_ml in CI_METHODS
                                )
                            ):
                                omitted_ci_rows.append({
                                    "ml_model": fair_method_ml.rsplit("-", 1)[-1],
                                    "utility_metric": UTILITY_METRIC,
                                    "dataset": normalized_dataset,
                                    "combo": current_combo,
                                    "fairness_metric": FAIRNESS_METRIC,
                                    "method": fair_method_ml,
                                    "epsilon": eps,
                                    "x_ci95": xerr,
                                    "y_ci95": yerr,
                                    "reason": "CI exceeds threshold or is invalid",
                                })
                ...
                output_dataset_name = _normalize_dataset_name(
                    normalized_dataset
                )
    ...
    print("[DONE] All plots completed.")

    if len(omitted_ci_rows) > 0:
        omitted_ci_df = pd.DataFrame(omitted_ci_rows)

        omitted_ci_file = (
            "omitted_ci95_large_intervals_all_models.csv"
        )

        omitted_ci_df.to_csv(
            omitted_ci_file,
            index=False,
        )

        print(
            f"[SAVED] Omitted CI information: "
            f"{omitted_ci_file}"
        )