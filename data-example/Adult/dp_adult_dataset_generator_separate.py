...
SEED = None
if len(sys.argv) != 4:
    print("Syntax: dp_dataset_generator.py eps seed storage-path")
    os.abort()
    
EPSILON = int(sys.argv[1])
SEED = int(sys.argv[2])
SAVE_PATH = sys.argv[3] + DP_ALG + "/"

print(f"Running dataset generator with seed {SEED} and eps {EPSILON}")
if SEED is not None:
    random.seed(SEED)
    np.random.seed(SEED)
    ...
    name = save_path+'split_train_dataset_seed_'+str(SEED)+'_eps_'+str(EPSILON)+'_.csv'
    ...
    for e in [EPSILON]:
        ...
        synth = Synthesizer.create(DP_ALG, verbose=False, epsilon=e) if rounds is None else Synthesizer.create(DP_ALG, verbose=False, epsilon=e, rounds=rounds)
        synth.fit(
            nf, preprocessor_eps = e/2,
            categorical_columns  = categorical_columns,
            ordinal_columns      = ordinal_columns,
            continuous_columns   = list(continuous_columns.keys()),
        )
